...
searchElements.forEach(element => {
    element.addEventListener("keyup", function() {
        ...
        typingTimer = setTimeout(async () => {
           ...
           // Wait for the response to resolve before proceeding
           const blogTab = document.querySelector('.search-tabs-nav .search-tab-item[data-tab="blogs"]');

            if (blogTab) {

                // Get the span inside the second child div
                const spanInSecondDiv = blogTab.querySelector('span');

                if (spanInSecondDiv) {
                    try {
                        // Await the response to get the JSON data
                        const { count, html_res } = await response.json();

                        // Update the blog container content
                        const blogContainer = document.getElementById('blogs');
                        if (blogContainer) {
                            blogContainer.innerHTML = html_res;
                        } else {
                            console.error('Blog container element not found');
                        }

                        // Update the second child's span with the count
                        spanInSecondDiv.innerHTML = count;
                    } catch (error) {
                        console.error('Error processing response:', error);
                    }
                } else {
                    console.error('Span element not found inside the second child div');
                }
            } else {
                console.error('Parent element or second child div not found');
            }

            // console.log(count,html_res);
